...
bool RegisterRayWavesFileAssociation()
{
    ...
    std::string exeQuoted = "\"" + exePath + R"(" "%1")";
    ...
    RegSetValueExA(hKey, nullptr, 0, REG_SZ, reinterpret_cast<const BYTE*>("RayWaves.Project"),
    ...
    // DefaultIcon
    result = RegCreateKeyExA(HKEY_CURRENT_USER,
        R"(Software\Classes\RayWaves.Project\DefaultIcon)",
        0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &hKey, nullptr);
    if (result != ERROR_SUCCESS) return false;
    std::string iconPath = exePath + ",0";
    RegSetValueExA(hKey, nullptr, 0, REG_SZ, reinterpret_cast<const BYTE*>(iconPath.c_str()),
        static_cast<DWORD>(iconPath.size() + 1));
    RegCloseKey(hKey);

    // shell\open\command
    result = RegCreateKeyExA(HKEY_CURRENT_USER,
        R"(Software\Classes\RayWaves.Project\shell\open\command)",
        0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &hKey, nullptr);
    if (result != ERROR_SUCCESS) return false;
    RegSetValueExA(hKey, nullptr, 0, REG_SZ, reinterpret_cast<const BYTE*>(exeQuoted.c_str()),
...
bool IsRayWavesFileAssociationRegistered()
{
    ...
    result = RegQueryValueExA(hKey, nullptr, nullptr, nullptr,

...
Copy-Item "$BuildPath/libEngine.a" "$DistPath/" -Force

# Copy raylib files to raylib folder structure
Copy-Item "$BuildPath/libraylib.dll" "$DistPath/raylib/bin/" -Force
Copy-Item "$BuildPath/_deps/raylib-build/raylib/libraylib.dll.a" "$DistPath/raylib/lib/libraylib.a" -Force
...
New-Item -ItemType Directory -Path "$DistPath/Tools/zig" -Force | Out-Null
Copy-Item "Tools/zig/*" "$DistPath/Tools/zig/" -Recurse -Force
...
Write-Host "  - lib/ (libraylib.dll.a)" -ForegroundColor White
Write-Host "  - bin/ (libraylib.dll)" -ForegroundColor White
